...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (staffTableBody) {
        ...
        const apiUrl = '../Backend/api/staff.php';
        fetch(apiUrl)
            ...
            .then(data => {
                ...
                data.forEach(user => {
                    // Map Depot_ID from Backend to Depot Name
                    let loc = "Headquarters"; 
                    if (user.Depot_ID == 1) loc = "Ha Noi";
                    else if (user.Depot_ID == 2) loc = "Da Nang";
                    else if (user.Depot_ID == 3) loc = "Ho Chi Minh City";
                    else if (user.Depot_ID == 4) loc = "Can Tho";

                    // No Status column in DB, default to Active for now
                    let statusBg = '#e8f5e9';
                    ...
                    let row = `
                      <tr style="border-bottom: 1px solid #eee;">
                        <td style="padding: 14px 20px; font-weight: 600;">${user.Full_Name}</td>
                        <td style="padding: 14px 20px;">${user.Role_Type}</td>
                        <td style="padding: 14px 20px;">${loc}</td>
                        <td style="padding: 14px 20px;">
                          <span style="background: ${statusBg}; color: ${statusColor}; padding: 4px 10px; border-radius: 12px; font-size: 12px; font-weight: 500;">
                            ${statusText}
                          </span>
                        </td>
                        <td style="padding: 14px 20px;">
                          <a href="#" class="edit-btn" data-id="${user.Staff_ID}" style="color: #007bff; text-decoration: none; margin-right: 12px; font-weight: 500;">Edit</a>
                          <a href="#" class="disable-btn" data-id="${user.Staff_ID}" style="color: #dc3545; text-decoration: none; font-weight: 500;">Disable</a>
                        </td>
                      </tr>
                    `;
